...
def main_worker(local_rank,world_size,rank_offset,config):
    print('main_work local_rank: ', local_rank)
    ...
    # train_split, tmp = torch.utils.data.random_split(train_dataset, [len(train_dataset)//180, len(train_dataset)-len(train_dataset)//180])
    # validate_split, tmp = torch.utils.data.random_split(validate_dataset, [len(validate_dataset)//180, len(validate_dataset)-len(validate_dataset)//180])

    # train_split_sampler=torch.utils.data.distributed.DistributedSampler(train_split)
    # validate_split_sampler=torch.utils.data.distributed.DistributedSampler(validate_split)

    # train_split_loader=torch.utils.data.DataLoader(train_split, batch_size=config.batch_size,sampler=train_split_sampler,pin_memory=True,num_workers=config.workers)
    # validate_split_loader=torch.utils.data.DataLoader(validate_split, batch_size=config.batch_size,sampler=validate_split_sampler,pin_memory=True,num_workers=config.workers)
    # print(train_dataset[0])
    print("finish datapreprocess")
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        # train_split_sampler.set_epoch(epoch)
        # train_split_sampler.dataset.next_epoch()
        # run(logs=logs,data_loader=train_split_loader,net=net,optimizer=optimizer,local_rank=local_rank,config=config,AG=AG,phase='train')
        # run(logs=logs,data_loader=validate_split_loader,net=net,optimizer=optimizer,local_rank=local_rank,config=config,phase='valid')
